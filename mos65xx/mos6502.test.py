...
from mos65xx import SparseMemory, mos6502


def machine(program, at=0x8000, decimal=True, **registers):
...
class ResetTest(unittest.TestCase):
    ...
    def test_a_reset_leaves_the_registers_holding_what_they_held(self):
    ...
    def test_the_same_seed_gives_the_same_unclean_machine(self):
        memory = SparseMemory(seed=3)
...
class StatusTest(unittest.TestCase):
    def test_the_unused_bit_is_always_set(self):
        cpu, _ = machine([0xEA])

        self.assertTrue(cpu.status() & 0x20)

    def test_the_break_bit_is_kept_when_nothing_touches_it(self):
        cpu, _ = machine([0xEA])
        cpu.set_status(0xFF)
    ...
    def test_pulling_the_status_always_clears_the_break_bit(self):
    ...
    def test_pushing_the_status_sets_the_break_bit(self):
        cpu, memory = machine([0x08])
        cpu.step()

        self.assertEqual(memory.read8(0x01FD), cpu.status() | 0x10)

    def test_pulling_the_status_takes_every_other_bit_as_given(self):
        cpu, memory = machine([0x28], s=0xFC)
...
class AddressingTest(unittest.TestCase):
    def test_a_zero_page_index_wraps_inside_the_first_page(self):
    ...
    def test_an_absolute_index_does_not_wrap_inside_a_page(self):
    ...
    def test_an_indexed_indirect_pointer_wraps_inside_the_first_page(self):
    ...
    def test_an_indirect_indexed_pointer_wraps_the_same_way(self):
    ...
    def test_a_jump_through_a_pointer_at_a_page_end_reads_the_wrong_high_byte(self):
...
class SubroutineTest(unittest.TestCase):
    def test_a_call_pushes_the_address_of_its_own_last_byte(self):
    ...
    def test_a_call_whose_push_lands_on_its_own_operand_jumps_where_the_push_left_it(self):
    ...
    def test_a_return_goes_to_the_byte_after_the_call(self):
...
class StackTest(unittest.TestCase):
    ...
    def test_a_pull_wraps_the_same_way(self):
...
class ArithmeticTest(unittest.TestCase):
    def test_an_add_sets_carry_when_it_leaves_the_byte(self):
    ...
    def test_an_add_sets_overflow_when_two_positives_make_a_negative(self):
    ...
    def test_a_decimal_add_carries_at_ten_rather_than_sixteen(self):
    ...
    def test_a_part_with_no_decimal_mode_adds_in_binary_regardless(self):
    ...
    def test_a_decimal_subtract_borrows_at_ten(self):
...
class UndocumentedTest(unittest.TestCase):
    def test_the_shift_and_or_does_both(self):
    ...
    def test_the_store_of_both_registers_stores_their_conjunction(self):
    ...
    def test_the_load_of_both_registers_loads_both(self):
    ...
    def test_the_unstable_transfer_uses_the_magic_the_suite_measured(self):
    ...
    def test_a_jam_stops_the_processor_where_it_stands(self):
        cpu, _ = machine([0x02])
    ...
    def test_a_stopped_processor_refuses_to_step_again(self):
        cpu, _ = machine([0x02])
        cpu.step()
...
class BranchTest(unittest.TestCase):
    def test_a_branch_not_taken_only_costs_its_operand(self):
    ...
    def test_a_branch_taken_goes_where_it_says(self):
        cpu, _ = machine([0xD0, 0x10])
        cpu.z = False
    ...
    def test_a_backward_branch_is_signed(self):
...
class InterruptTest(unittest.TestCase):
    def test_a_break_goes_through_the_vector_and_pushes_the_break_bit(self):
    ...
    def test_a_break_pushes_the_address_after_its_padding_byte(self):
        cpu, memory = machine([0x00, 0x00])
...
class LimitTest(unittest.TestCase):
    def test_a_run_that_never_ends_is_stopped_rather_than_hanging(self):
...
class EveryOpcodeTest(unittest.TestCase):
    ...
    def test_every_opcode_runs_in_every_state(self):
    ...
    def test_every_opcode_has_an_implementation_behind_it(self):
        missing = [
            ...
            for opcode, (mnemonic, _) in enumerate(mos6502.NMOS)
            if not hasattr(mos6502.Cpu, f"op_{mnemonic}")
        ]

        self.assertEqual(missing, [])

    def test_a_mode_no_instruction_uses_is_refused_rather_than_guessed(self):
        cpu, _ = machine([0xEA])

        with self.assertRaises(mos6502.UnsupportedError):
            cpu.effective("nonsense")

    def test_an_opcode_with_no_handler_is_refused_rather_than_skipped(self):
        cpu, _ = machine([0x00])
...
class ReachTest(unittest.TestCase):
    def test_a_routine_runs_until_it_returns(self):
    ...
    def test_a_nested_call_returns_to_the_outer_one(self):
    ...
    def test_running_until_a_condition_stops_when_it_holds(self):
    ...
    def test_an_accumulator_operand_is_the_accumulator(self):
        cpu, _ = machine([0xEA], a=0x42)

        self.assertEqual(cpu.operand("accumulator"), 0x42)

    def test_a_decimal_add_whose_low_nibble_stays_below_ten_does_not_adjust_it(self):
    ...
    def test_a_decimal_rotate_adjusts_both_halves_when_both_run_over(self):
    ...
    def test_a_decimal_rotate_leaves_the_carry_clear_when_the_high_half_does_not(self):
    ...
    def test_a_zero_page_indirect_reads_a_pointer_without_indexing(self):
