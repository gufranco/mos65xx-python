...
class TableTest(unittest.TestCase):
    def test_every_byte_decodes_to_something(self):
        self.assertEqual(len(opcodes6502.NMOS), 256)

    def test_no_entry_names_a_mode_the_table_cannot_size(self):
        for mnemonic, mode in opcodes6502.NMOS:
            self.assertIn(mode, opcodes6502.MODE_SIZE, mnemonic)

    def test_the_documented_instructions_are_all_present(self):
        documented = {
    ...
    def test_every_unstable_instruction_is_also_undocumented(self):
        self.assertTrue(opcodes6502.UNSTABLE <= opcodes6502.UNDOCUMENTED)

    def test_the_load_immediate_is_where_it_has_always_been(self):
        self.assertEqual(opcodes6502.NMOS[0xA9], ("lda", "immediate"))

    def test_the_jump_indirect_is_where_it_has_always_been(self):
        self.assertEqual(opcodes6502.NMOS[0x6C], ("jmp", "indirect"))

    def test_the_undocumented_subtract_shares_an_opcode_row_with_the_real_one(self):
        self.assertEqual(opcodes6502.NMOS[0xEB], opcodes6502.NMOS[0xE9])


class SizeTest(unittest.TestCase):
    def test_an_implied_instruction_carries_no_operand(self):
        self.assertEqual(opcodes6502.operand_size("implied"), 0)

    def test_a_zero_page_operand_is_one_byte(self):
        self.assertEqual(opcodes6502.operand_size("zeroPage"), 1)

    def test_an_absolute_operand_is_two(self):
        self.assertEqual(opcodes6502.operand_size("absolute"), 2)


class BranchTest(unittest.TestCase):
    def test_a_forward_branch_counts_from_the_instruction_after_it(self):
        self.assertEqual(opcodes6502.branch_target(0x1000, 2, 0x10), 0x1012)

    def test_a_backward_branch_is_a_signed_offset(self):
        self.assertEqual(opcodes6502.branch_target(0x1000, 2, 0xF0), 0x0FF2)

    def test_a_branch_past_the_top_wraps_within_the_address_space(self):
        self.assertEqual(opcodes6502.branch_target(0xFFF0, 2, 0x40), 0x0032)


class DecodeTest(unittest.TestCase):
    def test_an_instruction_reports_where_it_was_found(self):
    ...
    def test_an_absolute_operand_is_read_low_byte_first(self):
        found = opcodes6502.decode(bytes([0xAD, 0x34, 0x12]), 0, 0x8000)

        self.assertEqual(found.operand, 0x1234)

    def test_an_opcode_with_no_bytes_after_it_is_refused(self):
...
class RenderTest(unittest.TestCase):
    ...
    def test_the_two_indirect_zero_page_modes_read_differently(self):
    ...
    def test_an_absolute_indirect_is_parenthesised(self):
        self.assertEqual(opcodes6502.render("indirect", 0x1234, 0, 3), "($1234)")

    def test_every_mode_the_table_uses_renders(self):
...
class DisassembleTest(unittest.TestCase):
    def test_a_run_of_instructions_comes_back_in_order(self):
    ...
    def test_each_instruction_carries_the_address_it_sits_at(self):
    ...
    def test_addresses_wrap_at_the_top_of_the_space(self):
