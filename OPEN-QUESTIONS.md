...
**What this project does.** Spends an ordinary internal cycle, whatever the interrupt line is doing. Three widely used implementations do something else: ares and bsnes replace that cycle with a read of the program counter, without incrementing it, for a named list of two cycle opcodes, the flag instructions, the register transfers, the increments and decrements, the shifts and rotates, NOP and XCE.

The third is an FPGA core rather than an emulator, and it is worth reading because it states the same behaviour as a condition rather than as a list. In `SNES_MiSTer` at `rtl/65C816`, revision ef6aec3 read on 2026-08-24, the cycle asserts VPA when three things hold at once: it is the instruction's last cycle, its index is one, and the microcode marks it as carrying no valid address. VPA on its own is a program-counter read and the counter does not advance, which is the emulators' behaviour exactly. Walking that core's own microcode table, the condition resolves to thirty-one opcodes: the four shifts and rotates on the accumulator, the seven flag instructions, the twelve register transfers, the six increments and decrements, NOP and XCE. That is the emulators' list, arrived at from a rule rather than copied, which is a reason to think the list is not arbitrary.

**What would settle it.** A logic capture of a real part running one of those opcodes with the interrupt line already low. An implementation is a lead here rather than an authority, and a weaker one than three sources sounds: ares and bsnes share a lineage, and the FPGA core's author states having compared against bsnes while writing it. All three corroborate one reading of the part rather than three readings of it, and none ships the measurement behind the behaviour. A later reader hunting for a second source should know that another implementation from the same community is unlikely to be one, because the ones that exist were built against that emulator.
